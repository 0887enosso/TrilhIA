...
- **Streaks / foguinho (ofensiva):** monitor de engajamento com o **desafio diário**, não com "uso do app" em geral — avança 1 a cada dia em que o colaborador conclui o desafio diário (as 5 questões), e **zera** se um dia passar sem concluir nenhum. Responder questões normais de módulo não move o foguinho. Ver `src/lib/streak.ts` (`atualizarStreak`, chamada só por `processarRespostaParaDesafioDiario` em `src/lib/desafioDiario.ts`).
- **Streak freeze:** perdão de 1 dia sem quebrar o foguinho (aplica-se a um dia sem concluir o desafio diário), para reduzir abandono por frustração.
...
- **Ranking / Ligas semanais:** grupos de usuários competem por XP da semana, com sistema de subida/descida de liga.
  ...
  - **Contas de teste:** um usuário marcado como conta de teste (`Usuario.contaTeste`, alternável no painel admin) continua ganhando XP/nível/foguinho/conquistas normalmente, mas nunca entra em nenhuma `ParticipacaoLiga` — não compete (nem distorce a competição) contra colegas reais. Ver `adicionarXpSemanal` em `src/lib/ligas.ts`.
